...
Ghost::Ghost()
{
    ...
    timer_backtracking = new QTimer;
    connect(timer_backtracking,SIGNAL(timeout()),this,SLOT(Backtracking_Start())); //conect method that repeats the method everytime it recives the signal
    timer_backtracking->start(70); //Signal every 50 miliseconds

}
void Ghost::Backtracking_Start(){
    BackTracking bck;
    QPoint *current= new QPoint(position_x,position_y); // Punto actual
    QPoint target(posX_Pacman,posY_Pacman); // Punto destino inicial
    int maxDepth = 50; // Profundidad máxima para el backtracking

    postogo  ={current->x(),current->y()};
    bck.determineNextMove(*current,target,maxDepth);
    std::cout << "Movimiento actual: (" << current->x() << ", " << current->y() << ")\n";
