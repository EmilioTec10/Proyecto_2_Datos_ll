...
Levels::Levels(QWidget *parent)
{
    ...
    //timerr_set = new QTimer();
    //connect(timer_set,SIGNAL(timeout()),this, SLOT(SocketServer()));
    //timerr_set->start(3);


   // movementPacmanMobile = new QTimer();
    //connect(movementPacmanMobile,SIGNAL(timeout()) , &thread,SLOT(run()));
